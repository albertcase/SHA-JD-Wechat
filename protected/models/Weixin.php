...
class Weixin{
	...
	private $_TOKEN = 'jdwechat';
	private $_appid = 'wx6c50357d71f24c23';
	private $_secret = 'd23eea46f2889b4d8fed97f38dd20d95';
    ...
    public function responseMsg($postStr)
    {
		if (!empty($postStr)){
				...
				if($msgType=='text'){
					...
					$info_sql = "select * from `same_store` where lat<>0 and (lat between {$squares['right-bottom']['lat']} and {$squares['left-top']['lat']}) and (lng between {$squares['left-top']['lng']} and {$squares['right-bottom']['lng']})";
